...
pub async fn submit_tx(State(state) : State<AppState>, tx_bytes: Bytes) -> ApiResult<SubmitTxResponse> {
    ...
    tracing::info!(target: "node::server", size=?tx_bytes.len(), "tx size");
...
pub fn build_tx_outcome(db_handle: &DBHandle, envelope: TxEnvelope) -> Result<TxOutcome> {
    ...
    let receipt_opt: Option<Receipt>;
    match payload {
        TxPayload::Exec { ctr_addr_str, input, access_set} => {
            receipt_opt = exec_tx(&db_handle, &envelope, ctr_addr_str, input, access_set)
        },
        TxPayload::Deploy{ image_id, elf, elf_hash } => {
            receipt_opt = deploy_ctr(&envelope, image_id, elf, elf_hash)?
        },
        TxPayload::Update{ ctr_addr_str, image_id, elf, elf_hash } => {
            receipt_opt = update_ctr(&db_handle, &envelope, ctr_addr_str, image_id, elf, elf_hash)?
        },
    }
    Ok(TxOutcome::create(envelope, receipt_opt))
}
...
pub fn generate_response(outcome: &TxOutcome) -> Result<SubmitTxResponse> {
...
pub fn deploy_ctr(envelope: TxEnvelope, image_id: &Digest,
