...
pub fn init_logging() -> anyhow::Result<()> {
    ...
    let log_dir = proj_dir.data_local_dir().join("executor-logs");
...
pub fn init_env() -> anyhow::Result<()> {
    match dotenv() {
        Ok(path) => {
            tracing::debug!("Loaded environment variables from {:?} for executor", path);
            Ok(())
        },
        Err(e) if e.not_found() => {
            tracing::warn!("No .env found for executor");
            Ok(())
        },
        Err(e) => {
            tracing::error!("Failed to load .env file for executor: {}", e);
